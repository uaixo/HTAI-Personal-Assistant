...
// Packaged-app names derive from product-identity.cjs -- the same module
// electron-builder.config.cjs packs with -- so this script tracks
// productName / executableName / artifactName instead of pinning a brand.
// (It read package.json `build` until upstream moved packaging identity
// out of package.json and into product-identity.cjs.)
const PRODUCT_NAME = require('../product-identity.cjs').displayName
...
const APP = (() => {
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: unpacked
        ? path.join(unpacked, `${EXECUTABLE_NAME}.exe`)
        : path.join(RELEASE_ROOT, 'win-unpacked', `${EXECUTABLE_NAME}.exe`),
