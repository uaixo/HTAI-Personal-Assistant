...
const variants = {
  // Fork carve-out (uaixo/HTAI-Personal-Assistant, user-approved 2026-07-22):
  // the default variant ships as NousAI. This is where the brand values that
  // used to live in apps/desktop/package.json `build` (productName /
  // executableName / artifactName) now come from — upstream moved packaging
  // identity into this file and deleted that block. Upstream's `light` and
  // `bundled` variants are left untouched; this fork does not build them.
  '': { display: 'NousAI', kebab: 'nousai', pascal: 'NousAI' },
  light: {
...
const identity = {
  ...
  light,
  displayName,
  // Fork carve-out: the NousAI desktop app keeps its historical bundle
  // identifier (apps/desktop/package.json `build.appId` before the move).
  // Canary / commit / non-default variants keep upstream's derivation so
  // side-by-side installs still differ on every OS-visible marker.
  appId: variant === '' && !kebabSuffix ? 'ai.nous.desktop' : `com.nousresearch.${name.kebab}${kebabSuffix}`,
