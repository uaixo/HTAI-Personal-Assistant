...
// electron-builder.config.cjs packs with -- so the fixtures track
// productName / executableName instead of pinning a hardcoded brand.
// (They read package.json `build` until upstream moved packaging identity
// out of package.json and into product-identity.cjs.)
const PRODUCT_NAME: string = createRequire(import.meta.url)('../product-identity.cjs').displayName
const EXECUTABLE_NAME: string = PRODUCT_NAME
