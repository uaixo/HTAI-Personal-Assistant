...
async function ensureRuntime(
  ...
  if (backend.kind === 'bootstrap-needed' && installShape() === 'bundled') {
    ...
    const bundledError: Error & { isBootstrapFailure?: boolean } = new Error(
      'This app bundles its own Hermes runtime, but the runtime files are missing or damaged. Reinstall NousAI Desktop to restore it.'
    )
...
function reportPrimaryRecoveryCrashLoop(code: number | null, signal: string | null): boolean {
  ...
  const message =
    'Hermes backend keeps crashing right after it restarts; not restarting it again. Relaunch NousAI Desktop.'
